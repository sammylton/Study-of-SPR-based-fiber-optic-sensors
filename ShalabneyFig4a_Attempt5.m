...
theta=[40,50];
b=costheta(nm,theta);
function cos=costheta(ri,theta)
    global np
    cos=sqrt(1-((sind(theta)).^2).*((np^2)/(ri^2)));
end
%}
%This is the final code for Fig4a. For normalised electric field.
clear;
tic
...
zm=z;
zm(z>=dm)=dm;%I don't know how it can be z>=dm and should not compulsarily be zm>=dm %> or >= makes no difference, maybe MATLAB has it inbuilt to check it as a condiition
...
bdm=(costheta(nm,theta)).*((360*nm/lambda).*dm);%to be used in calculation of r
bm=(costheta(nm,theta)).*((360*nm/lambda).*zm);%360->2*pi when cosd->cos, sind->sin
%bm(z>=dm)=bdm;% M1 becomes constant after z=dm 
ba=(costheta(na,theta)).*((360*na/lambda).*za);
qp=(cosd(theta)).*((sqrt(u))/np);
qa=(costheta(na,theta)).*((sqrt(u))/na);
qm=(costheta(nm,theta)).*(sqrt(u)/nm);
...
%{
function bj=costheta(ri,theta)
    global np
    bj=sqrt(1-((sind(theta)).^2).*((np^2)/(ri^2)));
end
%}