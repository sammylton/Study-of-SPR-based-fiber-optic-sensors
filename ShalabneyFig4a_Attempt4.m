% For normalised electric field. In ShalabneyFig4a3, I used different matrix manipulations.
...
% To make the second propagation matrix Identity before z=dm
za=z-dm;
za(za<=0)=0;

zm=z;
zm(z>=dm)=dm;%> or >= makes no difference
%I don't know how it can be z>=dm and should not compulsarily be zm>=dm 
%, maybe MATLAB has it inbuilt to check it as a condition


%_________________________________________________________________________________________________________________________________________________________
%block to find resonance theta for given values starts
%_________________________________________________________________________________________________________________________________________________________
tofindthetamin=linspace(0,90,1000000);%angle in prism(angle of incidence)
costheta=sqrt(1-((sind(tofindthetamin)).^2).*((np^2)/(nm^2)));%cos of angle in metal
b=(costheta).*((360*nm*dm)/lambda);%Beta j used in Shalabney paper %360->2*pi when cosd->cos, sind->sin
qp=(cosd(tofindthetamin)).*((sqrt(u))/np);%qj in prism (qj used in Shalabney paper)
qa=(sqrt(1-((sind(tofindthetamin)).^2).*((np^2)/(na^2)))).*((sqrt(u))/na);%qj in analyte %used cos of angle in analyte
...
thetamin = tofindthetamin(indexmin); 
%_________________________________________________________________________________________________________________________________________________________
